 ...
 * Auth: X-Device-Token (firmware) OR session auth (dashboard).
 */
export async function GET(
  ...
  // Try device-token auth first (firmware path)
...
// ──── PUT /api/devices/:deviceId/pzem-config ─────────────────────────────
/**
 * Updates PZEM source-mode configuration.
 * Auth: session only (dashboard — firmware never writes this).
 * Rejects MANUAL selection of an offline phase with HTTP 409.
 */
export async function PUT(
  ...
  try {
    ...
    if (mode === "manual" && manualSource !== null) {
      ...
      // Reject MANUAL selection of a currently-offline phase
